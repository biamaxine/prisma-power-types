 ...
 *
 * @example
 * type SectorRel = PrismaOrderByRelation<'sector', Sector, 'name' | 'createdAt'>;
 *
 * // resultado:
 * // type SectorRel = { sector: { name?: SortOrder; createdAt?: SortOrder } };
 */
export type PrismaOrderByRelation<
 ...
 * @template R - Um array de `PrismaOrderByRelation` para incluir ordenação por relacionamentos (padrão: []).
 */
export type PrismaElementOrderBy<
  T extends PrismaElement,
  K extends keyof T,
  R extends PrismaOrderByRelation[] = [],
> = Prettify<
  {
    [P in K]?: null extends T[P] ? SortOrderInput | SortOrder : SortOrder;
  } & (R extends Array<infer Relation>
    ? {
        [P in Relation as P extends PrismaOrderByRelation<infer Name>
          ? Name extends K
            ? never
            : Name
          : never]?: P extends PrismaOrderByRelation<
          string,
